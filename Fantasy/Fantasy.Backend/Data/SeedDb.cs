...
public class SeedDb
{
    ...
    {
        await CheckUserAsync("Marcelo", "Zuluaga", "marcelofuentes_10@hotmail.com", "12345678", "Marcelo.jpg", UserType.Admin);
    ...
    private async Task<User> CheckUserAsync(string firstName, string lastName, string email, string phone, string image, UserType userType)
    {
        ...
        if (user == null)
        {
            ...
            var country = await _context.Countries.FirstOrDefaultAsync(x => x.Name == "Guatemala");
            user = new User
            {
                FirstName = firstName,
                LastName = lastName,
